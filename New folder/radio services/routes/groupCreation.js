...
var groupsModel = require('../mongodb/schemas/Events')
//var groupNotifications = require('../mongodb/schemas/Notifications')
// router.get('/getusers',function(req,res){
//     groupsModel.aggregate([{ "$match": { "users.type": "user" } },{ "$unwind": "$users" }, { "$match": { "users.type": "user" }},{ "$group": {
//         "_id": "$_id","users": { "$push": "$users" } }}],function(err,users){
//     if(err){
//         res.send("ERROR")
//     }
//     else{
...
router.get('/getusers',function(req,res){
  ...
  {$unwind: "$users"},{$group: {_id:"$_id",groupName : { $first: '$groupName' },category : { $first: '$category' },users: {$push:"$users"}, size: {$sum:1}}},{$sort:{size:-1}}],function(err,users){
    if(err){
        res.send("ERROR")
    }
    else{
...
// router.get('/getusers',function(req,res){
//     var input=req.query;
//     groupsModel.find({ category: { $ne: "guruswamy" },"users.userId":{$ne:input.userId}} ,function(err,users){
//     if(err){
//         res.send("ERROR")
//     }
//     else{
...
// router.get('/getusers',function(req,res){
//     var input=req.query;
//     groupsModel.find({ "users.userId":input.userId  },function(err,users){
//     if(err){
//         res.send("ERROR")
//     }
//     else{
...
//                    groupsModel.findById({_id:input.id}).

//                    populate({ path: 'userId' }).exec(function (err, groupdata) {
//                     if (err) {
...
router.get('/users', function (req, res) {
    var input=req.query;
    groupsModel.findById({_id:input.id}).select('users').populate('users.userId').exec( function (err, user) {
